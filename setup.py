...
class CMakeBuildExt(build_ext):
    ...
    def configure(self, ext: CMakeExtension) -> None:
        ...
        build_tool = []
        if _is_ascend():
            # CANN's direct-kernel host-stub generator compares object paths
            # literally; Ninja normalizes TARGET_OBJECTS paths differently
            # from compile_commands.json with CANN 9.0.1.
    ...
    def build_extensions(self) -> None:
        ...
        for ext in self.extensions:
            ...
            # MLAPO is an AscendC shared kernel linked by _C_ascend on A2/A3.
            # Preserve the sibling relationship required by its $ORIGIN rpath
            # in both editable installs and built wheels.
            if _is_ascend():
                kernel_patterns = [
                    f"{self.build_temp}/{VLLM_VENDOR}/libvllm_fl_ascend_kernels*.so",
                    f"{self.build_temp}/libvllm_fl_ascend_kernels*.so",
                ]
                built_kernel = next(
                    (match for pattern in kernel_patterns for match in glob.glob(pattern)),
                    None,
                )
                if built_kernel is None:
                    raise RuntimeError(
                        "Could not find built MLAPO kernel "
                        f"vllm_fl_ascend_kernels in {self.build_temp}"
                    )
                shutil.copy2(built_kernel, dest_path.parent / Path(built_kernel).name)
