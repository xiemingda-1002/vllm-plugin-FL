...
class WorkerFL(WorkerBase):
    ...
    def init_device(self):
        ...
        if current_platform.device_type == "npu":
            # Match rc1's build/runtime check only after this worker has chosen
            # its visible NPU. This also supplies the runtime generation for a
            # Python-only FL installation without native OPP metadata.
            from vllm_fl.platforms.ascend.hardware import (
                check_ascend_device_type,
            )
            from vllm_fl.platforms.ascend.runtime import (
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if self.cache_config.kv_cache_memory_bytes is None and hasattr(
            ...
            if (
                ...
                # Like rc1, expose measured categories after capture at INFO.
                # This is a next-launch suggestion, not a KV-cache resize.
                msg = msg.replace("CUDAGraph memory", "NPU graph memory")
                msg += (
                    f" After warmup: torch reserved memory "
                    f"{GiB(current_platform.torch_device_fn.memory_reserved(self.device))} "
                    f"GiB, torch allocated memory "
                    f"{GiB(current_platform.torch_device_fn.memory_allocated(self.device))} "
                    f"GiB."
                )
                # vLLM configures its own logger namespace; the FL worker
                # logger can otherwise inherit the root WARNING threshold.
                # Reuse its configured handler without changing other vendors.
                init_logger("vllm.vllm_fl.ascend.memory").info(msg)
            else:
                logger.debug(msg)
            ...
            if self.profiler is None:
                ...
                if profiler_type == "torch":
                    if (
                        ...
                        from vllm_fl.profiler.ascend import (
