...
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_cache import (
    AscendCompressorStateCache as CompressorStateCache,
    AscendDeepseekV4IndexerCache as DeepseekV4IndexerCache,
)
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_layer import (
...
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.mul_add import (
    muls_add_triton,
)
...
def _deepseek_v4_modelslim_prefix(prefix: str) -> str:
    """Map FL module prefixes back to DeepSeek ModelSlim descriptor names."""
    if prefix.startswith("model."):
        prefix = prefix.removeprefix("model.")
    elif prefix == "lm_head":
        return "head"
    prefix = prefix.replace("embed_tokens", "embed")
    prefix = prefix.replace(".self_attn.", ".attn.")
    prefix = prefix.replace(".mlp.", ".ffn.")
    prefix = prefix.replace(".input_layernorm", ".attn_norm")
    prefix = prefix.replace(
        ".post_attention_layernorm", ".ffn_norm"
    )
    prefix = prefix.replace(".down_proj", ".w2")
    return prefix
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        self.is_fusion_moe_shared_experts_enabled = bool(
            get_ascend_option("mix_placement", False)
        )
...
class DeepseekV4Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Keep the index-cache output buffer at model scope so all decoder
        # layers share one stable allocation.
...
class AscendDeepseekV4ForCausalLM(
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        ...
        return FusedMoE.make_expert_params_mapping(
            ...
            + (
                ...
                if get_ascend_option("mix_placement", False)
                else 0
            ),
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        rocm_aiter_moe_shared_expert_enabled = bool(
            get_ascend_option("mix_placement", False)
        )
