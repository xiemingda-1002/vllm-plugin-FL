...
def _static_norm_quant_patterns_available() -> bool:
    """Require the exact FL custom op used by static/bias pattern searches.

    Loading an FL dispatcher payload does not prove that it exports rc1's
    historical ``npu_add_rms_norm_bias`` producer and upstream
    ``vllm.quantize`` consumer. Do not trace or register those patterns until
    both exact operators are available.
    """
    if not enable_custom_op():
        return False
    return (
        hasattr(torch.ops._C_ascend, "npu_add_rms_norm_bias")
        and hasattr(torch.ops.vllm, "quantize")
    )
...
class AddRMSNormQuantFusionPass(VllmInductorPass):
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        for eps in common_epsilons:
            ...
            if _static_norm_quant_patterns_available():
