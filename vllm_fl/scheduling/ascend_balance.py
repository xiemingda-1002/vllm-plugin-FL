...
def _balance_scheduling_enabled(vllm_config) -> bool:
    # FL's compatibility boundary preserves rc1's additional-config-over-env
    # precedence, including the startup window before a worker owns a config.
    try:
        from vllm_fl.configs.ascend import (
            balance_scheduling_enabled,
        )

        return balance_scheduling_enabled(vllm_config)
    except Exception:
        pass
...
def apply_balance_scheduling_patch() -> None:
    """Compatibility activation entrypoint.

    The rc1 bindings above are intentionally installed at module import time;
    importing this module is therefore the idempotent activation operation.
    """
    return None
