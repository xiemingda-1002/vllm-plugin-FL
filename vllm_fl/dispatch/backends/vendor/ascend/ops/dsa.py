...
from vllm_fl.dispatch.backends.vendor.ascend.models.dsa_layer import DSAAttention
...
_DSA_FORWARD_REGISTERED = False


def ensure_dsa_forward_registered() -> None:
    """Register the graph-visible DSA entry point exactly once."""
    global _DSA_FORWARD_REGISTERED
    if _DSA_FORWARD_REGISTERED:
        return
...
def unfold_kvcache(kvcache):
    # Current vLLM's non-packed DSV4 cache reshape can put the one physical
    # cache tensor in a singleton tuple, then bind that tuple in a singleton
    # virtual-engine list.  Strip only singleton transport wrappers; a
    # multi-element tuple can be the semantic (K, scale) cache pair.
