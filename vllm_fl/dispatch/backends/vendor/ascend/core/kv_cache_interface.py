...
from .deepseek_v4_kv_cache import CompressAttentionManager
...
class AscendMLAAttentionSpec(MLAAttentionSpec):
    ...
    def storage_block_size(self) -> int:
        """Return the physical page width used by Ascend's compressed cache.

        The Ascend compressed-cache manager first converts logical tokens to
        compressed tokens and then allocates pages of ``block_size`` compressed
        slots.  Thus, unlike the upstream CUDA layout, ``block_size`` already
        describes the physical page width; applying ``compress_ratio`` here
        would divide that width a second time.  This matches rc1's dedicated
        runner path, which reshapes these pages with ``spec.block_size``.
        """
