...
The helpers read FL's ``VllmConfig.additional_config`` and local device-family
state to select the Ascend attention path.
"""
...
def using_paged_attention(
    ...
    An absent or empty ``pa_shape_list`` selects FIA. Paged attention is
    limited to explicitly listed shapes in ``FULL_DECODE_ONLY`` mode and is
    disabled for speculative decode.
    """
