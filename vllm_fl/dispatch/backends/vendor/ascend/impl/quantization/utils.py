...
from . import ASCEND_QUANTIZATION_METHOD
    ...
    try:
        if envs.VLLM_USE_MODELSCOPE:
            from modelscope.hub.file_download import (
                model_file_download,  # type: ignore[import-untyped]
            )
...
def detect_quantization_method(model: str, revision: str | None = None) -> str | None:
    ...
    """
    from . import MODELSLIM_CONFIG_FILENAME
...
def maybe_auto_detect_quantization(vllm_config) -> None:
    ...
    if detected != ASCEND_QUANTIZATION_METHOD:
        # This migration adds ModelSlim, not rc1's FP8/compressed-tensors
        # replacements. Leave FL's existing configuration untouched for those
        # checkpoint formats and for ordinary float checkpoints.
        return
