...
Ported from vLLM-Ascend v0.24.0rc1 (412cda26814f).
...
from .utils import calc_split_factor
...
def get_linear_quant_type(
    ...
    if proj_name in packed_modules_mapping:
        ...
        for shard_prefix in shard_prefixes:
            ...
            if shard_key not in quant_description and _is_missing_v_shard(
...
def create_scheme_for_layer(
    ...
    if layer_type == "linear":
        from .linear import create_linear_scheme

        return create_linear_scheme(quant_type)
    if layer_type == "moe":
        from .moe import create_moe_scheme

        return create_moe_scheme(quant_type)
    raise NotImplementedError(
        f"FL Ascend ModelSlim {quant_type} {layer_type} is not migrated"
    )
...
class AscendModelSlimConfig(QuantizationConfig):
    ...
    def override_quantization_method(
        cls, hf_quant_cfg, user_quant, hf_config: Any = None
    ) -> str | None:
        if hf_quant_cfg is not None:
            quant_method = hf_quant_cfg.get("quant_method", None)
            if not quant_method and user_quant in (None, ASCEND_QUANTIZATION_METHOD):
    ...
    def apply_vllm_mapper(self, hf_to_vllm_mapper: WeightsMapper):
        ...
        if self.quant_description:
            self.quant_description = hf_to_vllm_mapper.apply_dict(
    ...
    def _has_quant_weight(
        self, prefix: str, packed_modules_mapping: Mapping[str, list[str]]
    ) -> bool:
        proj_name = prefix.split(".")[-1]
        if proj_name in packed_modules_mapping:
            return all(
                f"{prefix.replace(proj_name, shard_proj_name)}.weight"
                in self.quant_description
                for shard_proj_name in packed_modules_mapping[proj_name]
            )
    ...
    def quant_prefix_mapper(self, model_type: str, prefix: str) -> str:
        ...
        if model_type == "step3p5_mtp" and prefix.startswith("model.layers."):
            ...
            if not self._has_quant_weight(prefix, packed_modules_mapping):
                for candidate in (
                    prefix.replace("model.layers.", "language_model.model.layers.", 1),
                ):
    ...
    def get_quant_method(
        self, layer: torch.nn.Module, prefix: str, tid2eid=None
    ) -> QuantizeMethodBase | None:
        from .linear import AscendLinearMethod
        ...
        if isinstance(layer, LinearBase):
            if self.is_layer_skipped_ascend(prefix, self.packed_modules_mapping):
                ...
                logger.debug(
                    "Select AscendUnquantizedLinearMethod for %s (layer=%s)",
                    prefix,
                    "LinearBase",
                )
            ...
            logger.debug(
                "Select AscendLinearMethod for %s (layer=%s)", prefix, "LinearBase"
            )
            ...
            raise NotImplementedError(
                "FL ModelSlim quantized attention is not migrated"
            )
        elif isinstance(layer, AttentionLayerBase) and self.is_c8_quant_layer(prefix):
            raise NotImplementedError("FL ModelSlim C8 KV cache is not migrated")
            ...
            if self.is_layer_skipped_ascend(prefix, self.packed_modules_mapping):
                ...
                logger.debug(
                    "Select AscendUnquantizedFusedMoEMethod for %s (layer=%s)",
                    prefix,
                    "FusedMoE",
                )
                return AscendUnquantizedFusedMoEMethod(layer.moe_config)
            from .moe import AscendFusedMoEMethod

            scheme = create_scheme_for_layer(
                self.quant_description, prefix, "moe", self.packed_modules_mapping
            )
            logger.debug(
                "Select AscendFusedMoEMethod for %s (layer=%s)", prefix, "FusedMoE"
            )
            return AscendFusedMoEMethod(scheme, layer.moe_config, tid2eid=tid2eid)
        elif isinstance(layer, VocabParallelEmbedding):
            if self.is_layer_skipped_ascend(prefix, self.packed_modules_mapping):
                logger.debug(
                    "Select UnquantizedEmbeddingMethod for %s (layer=%s)",
                    prefix,
                    "VocabParallelEmbedding",
                )
                return UnquantizedEmbeddingMethod()
            raise NotImplementedError(
                "FL ModelSlim quantized embedding is not migrated"
            )
    ...
    def is_layer_skipped_ascend(
        ...
        if proj_name in fused_mapping:
            ...
            for shard_prefix in shard_prefixes:
                ...
                if shard_key not in self.quant_description and _is_missing_v_shard(
    ...
    def is_fa_quant_layer(self, prefix):
        if self.enable_fa_quant:
            ...
            if (
                layer_id_str.isdigit()
                and int(layer_id_str) in self.kvcache_quant_layers
            ):
                return True
        return False
    ...
    def is_indexer_quant_layer(self, prefix):
        if self.enable_indexer_quant:
            ...
            if (
                layer_id_str.isdigit()
                and int(layer_id_str) in self.indexer_quant_layers
            ):
                return True
        return False
    ...
    def maybe_update_config(
        ...
        config_path = get_model_file(
            model_name, MODELSLIM_CONFIG_FILENAME, revision=revision
        )
