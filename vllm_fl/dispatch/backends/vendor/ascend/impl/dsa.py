...
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_abstract import (
    DSAAttentionImpl,
)
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_cache import (
    reshape_dsa_kv_cache,
)
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_linear import CVLinearWrapper
from vllm_fl.dispatch.backends.vendor.ascend.impl.dsa_rope import (
    get_cos_and_sin_dsa,
)
...
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.rms_norm import (
    triton_q_rms,
)
...
def _is_w8a8_dynamic_linear(linear: torch.nn.Module) -> bool:
    ...
    while quant_method is not None:
        if isinstance(quant_method, AscendModelSlimLinearMethod):
            return isinstance(
                quant_method.scheme, AscendW8A8DynamicLinearScheme
            )
        nested = getattr(quant_method, "quant_method", None)
        if nested is quant_method:
            break
        quant_method = nested
    return False
...
def split_decodes_and_prefills(
    ...
    """Find the decode/prefill boundary in a reordered batch."""
...
def _build_hadamard(
    ...
    *,
    ...
    """Build a Sylvester Hadamard matrix without an optional SciPy dependency."""
...
class AscendDSABackend(AttentionBackend):
    ...
    def reshape_kv_cache(
        ...
        return reshape_dsa_kv_cache(raw_tensor, kv_cache_spec, num_blocks)
...
class AscendDSAMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
    # DSA supports fixed-size decode batches captured by FULL_DECODE_ONLY.
    # Prefill and mixed-shape execution remain on the eager path.
    ...
    def __init__(
        ...
        if AscendDSAMetadataBuilder.hadamard is None:
            if hf_config.model_type == "deepseek_v4":
                ...
                dim_padded = 2**log_dim
                AscendDSAMetadataBuilder.hadamard = _build_hadamard(
                    dim_padded,
                    device=self.device,
                    dtype=torch.float,
                ).to(torch.bfloat16)
    ...
    def build(
        ...
        **kwargs,
        ...
        self.prefill_ratio_to_sas_metadata = kwargs.get(
            "prefill_ratio_to_sas_metadata", {}
        )
        self.decode_ratio_to_sas_metadata = kwargs.get(
    ...
    def build_decode_metadata(
        ...
        if self.decode_ratio_to_sas_metadata.get("query_start_loc", None) is None:
            ...
            if positions_cpu is None:
                positions_cpu = common_attn_metadata.positions.cpu()
            input_positions_cpu = positions_cpu[: self.num_decode_tokens].long()
    ...
    def build_for_drafting(
        ...
        **kwargs,
        ...
        assert self.compressor_ratio <= 1, (
            "Ascend DSA currently supports only the SWA layer for this path."
        )
    ...
    def build_prefill_metadata_for_drafting(
        ...
        **kwargs,
        ...
        prefill_slot_mapping = self.spec_slot_mapping[draft_step - 1][
...
class AscendDSAImpl(DSAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.multistream_dsa_preprocess = bool(
            get_ascend_option("multistream_dsa_preprocess", False)
        )
        self.multistream_dsv4_dsa_overlap = bool(
            get_ascend_option("multistream_dsv4_dsa_overlap", False)
        )
        self.prefill_comm_compute_overlap = bool(
            get_ascend_option("prefill_comm_compute_overlap", False)
        )
    ...
    def _mla_prolog_multistream(self, hidden_states, cos, sin, swa_kv_cache, slot_mapping, is_prefill=False):
        ...
        is_w8a8 = _is_w8a8_dynamic_linear(self.wq_b)
    ...
    def _forward_decode(
        ...
        if self.multistream_dsv4_dsa_overlap:
            ...
            # q
            if _is_w8a8_dynamic_linear(self.wq_b):
    ...
    def _indexer_qkv_prepare(
        ...
        if (
            _is_w8a8_dynamic_linear(self.inderxer_wq_b)
