...
from .compat import get_ascend_config
from vllm_fl.ascend_forward_context import _EXTRA_CTX, MoECommType
from vllm_fl.dispatch.backends.vendor.ascend.impl.moe.experts_selector import select_experts, zero_experts_compute
from vllm_fl.dispatch.backends.vendor.ascend.impl.moe.moe_comm_method import AllGatherCommImpl, FusedExpertsResult, setup_moe_comm_method
from vllm_fl.dispatch.backends.vendor.ascend.impl.moe.moe_runtime_args import build_fused_experts_input
from .compat import get_moe_num_logical_experts
...
class AscendMoERunner(MoERunner):  # type: ignore[no-redef]
    ...
    def __init__(
        ...
        if ascend_config.enable_fused_mc2:
            ...
            if self.quant_type not in (QuantType.NONE, QuantType.W8A8):
                raise NotImplementedError(
                    "FL Ascend fused MC2 supports unquantized BF16/FP16 "
                    "and ModelSlim W8A8_DYNAMIC"
                )
            if get_ascend_device_type() is not AscendDeviceType.A3:
                raise NotImplementedError(
                    "FL Ascend fused MC2 is packaged only for A3"
                )
        ...
        if (
            ...
            raise NotImplementedError("FL Ascend rc1 MoE EPLB is not migrated")

        self.enable_npugraph_ex_static_kernel = False
        ...
        # Preserve rc1 runner state while using the expert map allocated by the
        # current vLLM factory.  EPLB/redundant placement is rejected above.
        AscendMoERunner.moe_counter += 1
        self.moe_instance_id = AscendMoERunner.moe_counter
        self.global_expert_map = None
        self._expert_map = routed_experts.expert_map
        self.log2phy = None
        self.global_redundant_expert_num = 0
        moe_config.global_redundant_expert_num = 0
        ...
        if self.multistream_overlap_shared_expert:
            # Validate after all routed/shared weights have been loaded and
            # post-processed: the overlap path splits the shared MLP at the
            # activation boundary, so it must remain equivalent to the normal
            # integrated shared-expert forward.
            original_process_weights = (
                self._quant_method.process_weights_after_loading
            )
            ...
            self._quant_method.process_weights_after_loading = (  # type: ignore
