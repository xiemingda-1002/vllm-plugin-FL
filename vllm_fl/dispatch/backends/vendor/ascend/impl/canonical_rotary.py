...
def npu_rotary_embedding(
    ...
    """Apply current Ascend RoPE dispatch with a functional tensor contract."""
    import torch_npu
    ...
    if HAS_TRITON:
        from .rope import rope_forward_triton

        # The native Triton implementation updates contiguous inputs in place.
        # This dispatcher op is functional (mutates_args=[]), so own its buffers.
        query, key = rope_forward_triton(
            query.contiguous().clone().view(num_tokens, -1, head_size),
            key.contiguous().clone().view(num_tokens, -1, head_size),
