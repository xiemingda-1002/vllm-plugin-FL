...
def solve_tril_16x16_kernel_v2(
    A,
    Ad,
    cu_seqlens,
    chunk_indices,
...
@triton.heuristics({"IS_VARLEN": lambda args: args["cu_seqlens"] is not None})
@triton.jit(do_not_specialize=["T", "H"])
def merge_16x16_to_32x32_inverse_kernel_v2(
    A,
    Ad,
    Ai,
    cu_seqlens,
...
def solve_tril(
    ...
    solve_tril_16x16_kernel_v2[NT, B * H](
        A=A,
        Ad=Ad,
    ...
    merge_fn = (
        merge_16x16_to_32x32_inverse_kernel_v2
        if BT == 32
        else merge_16x16_to_64x64_inverse_kernel_v2
    )
