...
"""rc1 Triton SwiGLU plus per-token int8 quantization kernel."""
...
def swiglu_quant(x, group_list, group_list_type, need_quant=True):
    """Run the rc1 kernel; callers preserve the native group-list contract."""
    if group_list_type not in [0, 1]:
        raise ValueError(
            f"swiglu_quant: group_list_type must be 0 or 1, but got {group_list_type}"
        )
    s, h = x.shape
    ...
    if group_list.dtype == torch.int64:
        ...
        raise ValueError(
            "swiglu_quant: group_list dtype must be torch.int32 or torch.int64, "
