...
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.fla.fused_qkvzba_split_reshape import fused_qkvzba_split_reshape_cat
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.fla.utils import clear_ssm_states
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.fused_gdn_gating import fused_gdn_gating_patch
from vllm_fl.dispatch.backends.vendor.ascend.impl.triton.mamba.causal_conv1d import causal_conv1d_fn


def get_graph_params():
    return get_ascend_graph_params()
...
def _runtime_num_actual_tokens(
    ...
    padding according to the Ascend GDN execution contract.
    """
...
def update_conv1d_graph_params(
    ...
    if is_draft_model:
        raise NotImplementedError("FL Ascend graph mode does not support draft models yet")
...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    def forward(
        ...
        # Keep the token dimension data-dependent under eager-FX tracing.
        # FULL_DECODE_ONLY traces during a one-token decode capture; building
        # this buffer from a Python shape tuple can freeze its first dimension
        # at 1 and then break the first 16K chunked-prefill step.  zeros_like
        # preserves vLLM's required zero initialization while deriving the
        # runtime token dimension directly from the projected tensor.
        value_width = (
            self.num_v_heads // self.tp_size * self.head_v_dim
        )
        core_attn_out = torch.zeros_like(mixed_qkv[:, :value_width]).reshape(
            -1,
            self.num_v_heads // self.tp_size,
            self.head_v_dim,
        )
    ...
    def _forward_core(
        ...
        num_actual_tokens = _runtime_num_actual_tokens(attn_metadata, mixed_qkv)
        # FULL_DECODE_ONLY may first trace the eager FX graph while a one-token
        # decode graph is being captured. Python scalar fields on the captured
        # metadata object can then retain that value even when the first real
        # request is a much larger chunked prefill. The helper uses the
        # unpadded runtime tensor shape for prefill and preserves metadata-based
        # decode padding.
        ...
        if attn_metadata.num_prefills > 0:
            ...
            # Dispatches to the vllm-ascend AscendC custom operator
            # (csrc/recurrent_gated_delta_rule), NOT built-in CANN.
