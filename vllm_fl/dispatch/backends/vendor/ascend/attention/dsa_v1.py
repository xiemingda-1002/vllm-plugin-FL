...
from vllm_fl.dispatch.backends.vendor.ascend.attention.abstract import DSAAttentionImpl
from vllm_fl.ascend_forward_context import _EXTRA_CTX
from vllm_fl.dispatch.backends.vendor.ascend.attention.attention_mask import (
...
from vllm_fl.dispatch.backends.vendor.ascend.ops.cv_linear import CVLinearWrapper
from vllm_fl.dispatch.backends.vendor.ascend.ops.rope_dsv4 import (
    get_cos_and_sin_dsa,
...
if TYPE_CHECKING:
    ...
    from vllm_fl.dispatch.backends.vendor.ascend.ops.triton.rms_norm import triton_q_rms

if HAS_TRITON:
    from vllm_fl.dispatch.backends.vendor.ascend.ops.triton.rms_norm import (
        triton_q_rms,  # noqa: F811
    )
else:
...
class AscendDSABackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        """Return the dtype-independent DSA KV layout.

        ``cache_dtype_str`` is accepted to match vLLM's current attention
        backend contract; it does not change the Ascend DSA cache layout.
        """
...
class AscendDSAMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
    ...
    def __init__(
        ...
        if (
            AscendDSAMetadataBuilder.hadamard is None
            and hf_config.model_type == "deepseek_v4"
        ):
                ...
                if self.vllm_config.model_config.enable_sleep_mode:
                    ...
                    from vllm_fl.dispatch.backends.vendor.ascend.device_allocator.camem import (
                        CaMemAllocator,
                    )
