...
from vllm_fl.dispatch.backends.vendor.ascend.attention.attention_mask import AttentionMaskBuilder
from vllm_fl.dispatch.backends.vendor.ascend.attention.utils import (
    SFA_QSFA_TILE_SIZE, AscendCommonAttentionMetadata,
    ascend_chunked_prefill_workspace_size, enable_cp,
    get_sfa_qsfa_packed_head_dim, maybe_save_kv_layer_to_connector,
    notify_kv_cache_written, trans_rope_weight, transdata,
    wait_for_kv_layer_from_connector,
)
from vllm_fl.dispatch.backends.vendor.ascend.device.device_op import DeviceOperator
from vllm_fl.dispatch.backends.vendor.ascend.device.mxfp_compat import FLOAT8_E8M0FNU_DTYPE
from vllm_fl.dispatch.backends.vendor.ascend.distributed.utils import all_gather_async
...
from vllm_fl.dispatch.backends.vendor.ascend.impl.quantization.linear import (
    AscendW8A8DynamicLinearMethod, AscendW8A8LinearMethod,
)
from vllm_fl.dispatch.backends.vendor.ascend.impl.rope import (
    get_cos_and_sin_mla,
    rope_forward_triton_siso,
)
from vllm_fl.dispatch.backends.vendor.ascend.memcache_comm_fence import (
    record_attention_compute_start,
)
from vllm_fl.dispatch.backends.vendor.ascend.dsa_compat import (
    AscendDeviceType, enable_dsa_cp, enable_dsa_cp_with_o_proj_tp,
    enable_sfa_dcp_replicated_indexer, enable_sp, get_ascend_device_type,
    maybe_trans_nz, round_up,
)
from vllm_fl.worker.npu_input_batch import NPUInputBatch

# P0 deliberately excludes CP/DCP and MTP. Keep their selector hooks lazy so
# a single-card eager GLM5.2 import never introduces those execution chains.
MLAPO_MAX_SUPPORTED_TOKENS = 1024
AscendPCPMetadata = Any
ACL_FORMAT_FRACTAL_ND = 2
ACL_FORMAT_FRACTAL_NZ = 29

def dispose_layer(layer: Any) -> None:
    """Release rc1's temporary projection tensors after their replacement."""
    for attr_name in dir(layer):
        attr_value = getattr(layer, attr_name)
        if isinstance(attr_value, torch.Tensor):
            attr_value.set_(
                torch.empty((0,), device=attr_value.device, dtype=attr_value.dtype)
            )
...
class AscendSFABackend(AttentionBackend):
    ...
    @staticmethod
    def get_builder_cls():
        if enable_sfa_dcp_replicated_indexer() or enable_cp():
            raise NotImplementedError(
                "FL Ascend SFA context-parallel cache and metadata are not migrated"
            )
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if not self.enable_sfa_prolog_v3 and self.enable_mlapo:
            ...
            reasons = []
            if quant_method.__class__.__name__ == "AscendW8A8MXFP8DynamicLinearMethod":
                raise NotImplementedError("FL Ascend SFA MXFP8 is outside GLM5.2 W8A8 P0")
            is_quantized = isinstance(quant_method, AscendW8A8LinearMethod)
