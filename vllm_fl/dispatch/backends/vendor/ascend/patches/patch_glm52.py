...
def _deepseek_v2_mla_attention_init(
    ...
    if getattr(config, "model_type", None) == "glm_moe_dsa":
        # rc1's Ascend rotary OOT class records the upstream interleaved
        # cache for SFA metadata. FL reuses the upstream rope class, so record
        # that same cache at the GLM-only MLA construction boundary.
        from vllm_fl.dispatch.backends.vendor.ascend.impl.rope import (
            record_cos_and_sin_cache_interleaved,
        )

        rope_cache = self.rotary_emb.cos_sin_cache
        record_cos_and_sin_cache_interleaved(rope_cache)
...
def apply_glm52_shared_indexer_patch() -> None:
    """Install rc1's GLM-5.2 shared-indexer-aware MLA constructor."""
    original_init = DeepseekV2MLAAttention.__init__
    if getattr(original_init, "_fl_glm52_shared_indexer", False):
        return

    @wraps(original_init)
    def scoped_init(self, vllm_config, config, *args, **kwargs):
        # The model class is shared with existing dense MLA models. Their
        # constructor must remain unchanged by the GLM checkpoint adaptation.
        init = (
            _deepseek_v2_mla_attention_init
            if getattr(config, "model_type", None) == "glm_moe_dsa"
            else original_init
        )
        return init(self, vllm_config, config, *args, **kwargs)

    scoped_init._fl_glm52_shared_indexer = True
    DeepseekV2MLAAttention.__init__ = scoped_init
