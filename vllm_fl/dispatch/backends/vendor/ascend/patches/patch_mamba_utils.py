...
"""Install FL's Ascend Mamba state-copy support in upstream vLLM.

The upstream kernel uses a pointer layout that the Ascend Triton compiler
cannot lower for an aligned Mamba cache copy. The FL-local kernel launches
with a larger block size that satisfies the Ascend Triton lowering contract.
"""
