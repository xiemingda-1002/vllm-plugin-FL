...
def _allocate_attention_output(
    layer_idx: int, hidden_states: torch.Tensor
) -> torch.Tensor:
    """Allocate the current-rc1 token-sharded FlashComm1 output buffer."""
    input_tokens = hidden_states.shape[0]
    flashcomm1_enabled = bool(_EXTRA_CTX.flash_comm_v1_enabled)
    tp_size = (
        get_tensor_model_parallel_world_size() if flashcomm1_enabled else 1
    )
    output_tokens = flashcomm1_attention_output_tokens(
        layer_idx=layer_idx,
        num_tokens=input_tokens,
        tp_size=tp_size,
        enabled=flashcomm1_enabled,
    )
    if output_tokens != input_tokens:
        return torch.empty(
            (output_tokens, hidden_states.shape[-1]),
...
class AscendQwen3_5DecoderLayer(Qwen3_5DecoderLayer):
    def forward(
        ...
        **kwargs: object,
        ...
        self_attention_output = _allocate_attention_output(
