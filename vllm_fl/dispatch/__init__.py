...
def _requires_compile_prewarm() -> bool:
    # Resolve only during tracing: importing dispatch also happens while the
    # platform plugin is being selected. An eager platform import here would
    # create a circular initialization dependency.
...
class CachedOp:
    ...
    def __call__(self, *args, **kwargs):
        # Ascend fullgraph tracing uses the explicit runner prewarm lifecycle.
        # Other vendors retain their existing cache/policy/fallback contract;
        # their runners do not opt into mandatory compile prewarm.
        if torch.compiler.is_compiling() and _requires_compile_prewarm():
            if not _OP_FAST_PATH_ENABLED:
                raise RuntimeError(
