...
MIN_CPUS_PER_NPU = (
    5  # 2(IRQ) + 1(main, at least 1 CPU) + 1(acl) + 1(release) = 5 CPUs per NPU
)
...
class AscendDeviceType(Enum):
    """Runtime device families needed by the CPU-binding policy."""
...
def get_ascend_device_type() -> AscendDeviceType:
    """Detect the active device without importing ``vllm_ascend``.

    vLLM-Ascend reads a wheel build marker and later checks it against the
    runtime SOC. FL ships A2 and A3 artifacts together, so an explicit
    ``SOC_VERSION`` selects the policy when present; otherwise the runtime SOC
    is queried directly.
    """

    soc_name = os.getenv("SOC_VERSION", "").strip().lower()
    if soc_name.startswith(("ascend910_93", "ascend910c")):
        return AscendDeviceType.A3
    if soc_name.startswith("ascend310p"):
        return AscendDeviceType._310P
    if soc_name.startswith("ascend950"):
        return AscendDeviceType.A5
    if soc_name.startswith(("ascend910b", "910b")):
    ...
    try:
        ...
        logger.debug("Unable to read Ascend runtime SOC version: %s", exc)

    logger.warning(
        "Unable to identify the Ascend device family; falling back to the "
        "A2 topology-affinity CPU-binding policy."
    )
    return AscendDeviceType.A2
...
class CpuAlloc:
    ...
    def build_global_slice_cpu_pool(self) -> None:
        ...
        def _slice_for_npu(global_npu_id: int) -> list[int]:
            ...
            start = global_npu_id * base + (
                global_npu_id if global_npu_id < extra else extra
            )
    ...
    def build_cpu_pools(self) -> None:
        ...
        if not self.device_info.npu_affinity:
            logger.warning(
                "NPU topo affinity not found, fallback to global-slice CPU binding."
            )
            self.build_global_slice_cpu_pool()
            return
        ...
        self.npu_cpu_pool = {
            npu: final[npu] for npu in self.device_info.running_npu_list
        }
    ...
    def print_plan(self) -> None:
        ...
        logger.info(
            "NPU%s: main=[%s]  acl=[%s]  release=[%s]", current_npu, main, acl, rel
        )
    ...
    def bind_memory(self, pid: str, npu: int) -> None:
        ...
        if not shutil.which("migratepages"):
            logger.info(
                "The 'migratepages' command is not available, skipping memory binding."
            )
            return
        target_numa = _get_npu_numa_node(npu)
        if target_numa is None:
            logger.warning(
                "[migrate] rank:%s -> NPU%s has no CPU pool, skip memory binding.",
                self.rank_id,
                npu,
            )
            return
        ...
        if target_numa not in all_numa_nodes:
            logger.warning(
                "[migrate] NPU:%s -> NUMA %s not found, skip memory binding.",
                npu,
                target_numa,
            )
            return
    ...
    def bind_npu_irq(self) -> None:
        ...
        if current_npu not in self.npu_cpu_pool:
            logger.warning(
                "[irq] rank:%s -> NPU%s has no cpu pool, skip irq binding.",
                self.rank_id,
                current_npu,
            )
            return
        ...
        if shutil.which("systemctl"):
            ...
            if "irqbalance.service" in output:
                _, return_code = execute_command(
                    ["systemctl", "is-active", "--quiet", "irqbalance"]
                )
                if return_code == 0:
                    logger.warning(
        ...
        if len(cpus) < 2:
            logger.warning(
                "[irq] NPU%s cpu pool too small (<2), skip irq binding.", npu
            )
            return
        ...
        try:
            ...
            logger.warning(
                "The msi_irqs folder cannot be found under /sys/bus/pci/devices/%s .",
                pci_addr,
            )
            return
