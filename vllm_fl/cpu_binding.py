...
def get_ascend_device_type() -> AscendDeviceType:
    """Detect the active Ascend device family from FL-owned runtime state.

    FL ships A2 and A3 artifacts together, so an explicit ``SOC_VERSION``
    selects the policy when present; otherwise the runtime SOC is queried
    directly.
    """
