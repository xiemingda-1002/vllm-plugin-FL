...
class AscendDeviceType(Enum):
    """Ascend generations used by the rc1 CPU-affinity policy."""
...
def get_ascend_device_type() -> AscendDeviceType:
    """Resolve the active generation without importing vllm-ascend.

    vLLM-Ascend reads this value from its build metadata. FL is an independent
    plugin and has no vllm-ascend package, so use torch-npu's runtime SOC id at
    the post-warmup binding point instead.
    """
    import torch_npu
