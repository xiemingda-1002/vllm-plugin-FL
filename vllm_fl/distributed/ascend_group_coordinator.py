...
from vllm_fl.distributed.device_communicators.npu_communicator import NPUCommunicator
from vllm_fl.distributed.ascend_hccl_registry import HcclPgKey, HcclPgRegistry, make_hccl_pg_key
from vllm_fl.distributed.ascend_hccl_options import create_hccl_pg_options
...
class GroupCoordinatorPatch(GroupCoordinator):
    def __init__(
        ...
        self._fl_non_hccl = _normalize_backend(torch_distributed_backend) != "hccl"
        if self._fl_non_hccl:
            super().__init__(
                group_ranks, local_rank, torch_distributed_backend,
                use_device_communicator, use_message_queue_broadcaster, group_name,
            )
            return
    ...
    def destroy(self):
        if getattr(self, "_fl_non_hccl", False):
...
def apply_ascend_distributed_patch() -> None:
    """Install rc1 HCCL coordination only from the Ascend vendor lifecycle."""
