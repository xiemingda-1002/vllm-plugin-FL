...
function(add_bin_compile_target)
    ...
    foreach(bin_script ${compile_scripts})
        ...
        if (_compile_flag)
            ...
            # CANN's value 1 runs inner make without an explicit -j setting.
            # Ninja has no such jobserver: allow an explicit, bounded inner
            # make -j setting without changing the default build behavior.
            if (DEFINED ENV{TILINGKEY_PARALLEL_JOB} AND NOT "$ENV{TILINGKEY_PARALLEL_JOB}" STREQUAL "")
                if (NOT "$ENV{TILINGKEY_PARALLEL_JOB}" MATCHES "^[1-9][0-9]*$")
                    message(FATAL_ERROR "TILINGKEY_PARALLEL_JOB must be a positive integer")
                endif ()
                list(APPEND _BUILD_COMMAND export TILINGKEY_PAR_COMPILE=0 &&)
                list(APPEND _BUILD_COMMAND export TILINGKEY_PARALLEL_JOB=$ENV{TILINGKEY_PARALLEL_JOB} &&)
            else ()
