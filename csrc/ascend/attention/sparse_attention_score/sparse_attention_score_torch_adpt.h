 ...
 */
#ifndef SPARSE_ATTENTION_SCORE_TORCH_ADPT_H  // NOLINT
...
namespace meta {
// The op is out-of-place and its output shape equals the query shape, so the
// Meta implementation only needs to allocate that shape.
at::Tensor npu_sparse_attention_score(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &value,
    const at::Tensor &selectIdx, const at::Tensor &blockTable,
    const c10::optional<at::Tensor> &selectNumIdx,
    ...
    int64_t innerPrecise)
{
    ...
    (void)actualSeqLengthsKv;
    (void)qInputLayout;
    (void)kvInputLayout;
    (void)numKeyValueHeads;
    (void)scaleValue;
    (void)blockSize;
    (void)topK;
    (void)innerPrecise;
    at::ScalarType outDtype = (query.scalar_type() == at::kFloat8_e4m3fn)
                                  ? at::kHalf
                                  : query.scalar_type();
    return at::empty_symint(query.sym_sizes(), query.options().dtype(outDtype));
}
}  // namespace meta
