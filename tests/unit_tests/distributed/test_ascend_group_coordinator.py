...
_PATCH_MODULE_PATH = _WORKTREE_ROOT / "vllm_fl/distributed/ascend_group_coordinator.py"
_PATCH_MODULE_NAME = "vllm_fl.distributed.ascend_group_coordinator"
_REGISTRY_MODULE_PATH = _WORKTREE_ROOT / "vllm_fl/distributed/ascend_hccl_registry.py"
_REGISTRY_MODULE_NAME = "vllm_fl.distributed.ascend_hccl_registry"
...
def _load_patch_distributed_module():
    ...
    class BaseGroupCoordinator:
        def __init__(self, *args):
            self.base_init_args = args
            self.base_destroy_calls = 0

        def destroy(self):
            self.base_destroy_calls += 1
    ...
    vllm_fl_module: Any = ModuleType("vllm_fl")
    vllm_fl_patch: Any = ModuleType("vllm_fl.patch")
    vllm_fl_patch_worker: Any = ModuleType("vllm_fl.patch.worker")
    vllm_fl_distributed: Any = ModuleType("vllm_fl.distributed")
    vllm_fl_device_communicators: Any = ModuleType("vllm_fl.distributed.device_communicators")
    npu_communicator_module: Any = ModuleType("vllm_fl.distributed.device_communicators.npu_communicator")
    utils_module: Any = ModuleType("vllm_fl.distributed.ascend_hccl_options")
    ...
    vllm_fl_module.patch = vllm_fl_patch
    vllm_fl_patch.worker = vllm_fl_patch_worker
    vllm_fl_module.distributed = vllm_fl_distributed
    vllm_fl_distributed.device_communicators = vllm_fl_device_communicators
    ...
    modules = {
        ...
        "vllm_fl": vllm_fl_module,
        "vllm_fl.patch": vllm_fl_patch,
        "vllm_fl.patch.worker": vllm_fl_patch_worker,
        "vllm_fl.distributed": vllm_fl_distributed,
        "vllm_fl.distributed.device_communicators": (vllm_fl_device_communicators),
        "vllm_fl.distributed.device_communicators.npu_communicator": (npu_communicator_module),
        "vllm_fl.distributed.ascend_hccl_options": utils_module,
    }
    ...
    try:
        ...
        # Import alone must not install an Ascend patch for other vendors.
        assert parallel_state_module.GroupCoordinator is BaseGroupCoordinator
        patch_module.apply_ascend_distributed_patch()
        installed_destroy = parallel_state_module.destroy_distributed_environment
        patch_module.apply_ascend_distributed_patch()
        assert parallel_state_module.destroy_distributed_environment is installed_destroy
...
def test_non_hccl_lifecycle_delegates_to_unmodified_upstream(module_env):
    group = _make_group(
        module_env,
        backend="nccl",
        group_name="tp",
    ...
    group.destroy()
    group.destroy()
    assert group.base_init_args[2] == "nccl"
    assert group.base_destroy_calls == 2
    assert module_env.new_group_calls == []
    module_env.current_device.assert_not_called()
    module_env.destroy_process_group.assert_not_called()
