...
def test_a3_bf16_fused_mc2_native_closure_and_dispatch_contract() -> None:
    root = ROOT / "csrc/ascend/mc2/dispatch_ffn_combine_bf16"
    files = sorted(path for path in root.rglob("*") if path.is_file())
    assert len(files) == 43

    # Current vLLM-Ascend 0.24.0rc1 source content, with the repository's
    # LF/final-newline normalization used for imported CANN sources.
    ...
    assert digest.hexdigest() == (
        "71975842c6ef5f2d2ad47665b8921a4433f6f06ea2543f9316d9b468c72e1da6"
    )

    dispatcher = _text(
        "csrc/ascend/mc2/dispatch_ffn_combine/dispatch_ffn_combine_torch_adpt.h"
    )
    assert "weight_dtype == at::kChar" in dispatcher
    assert "aclnnDispatchFFNCombine," in dispatcher
    assert "weight_dtype == at::kBFloat16" in dispatcher
    assert "aclnnDispatchFFNCombineBF16" in dispatcher
    assert "aclnnDispatchFFNCombineW4A8" not in dispatcher

    manifest = _text("csrc/ascend/build_opp.sh")
    assert "ascend910_93" in manifest
    assert "ops+=(dispatch_ffn_combine dispatch_ffn_combine_bf16)" in manifest
...
def test_qwen_gdn_backend_and_flaggems_off_provider_are_owned_by_fl() -> None:
    ...
    builder = _text(
        "vllm_fl/attention/ascend/gdn_attn_builder.py"
    )
    ...
    assert "from vllm_fl.dispatch.backends.vendor.ascend.patches import patch_qwen3_5" in patch
...
def test_tp2_hccl_and_bf16_moe_do_not_route_to_cuda_or_flaggems() -> None:
    ...
    assert "def process_weights_after_loading(self, layer: torch.nn.Module) -> None" in layer
