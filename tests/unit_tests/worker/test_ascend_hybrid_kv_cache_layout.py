...
def _runner(config, attention_layer, mamba_layer, attention_spec, mamba_spec):
    ...
    # The allocation and reshape paths check the GLM-SFA specialization before
    # entering the generic hybrid layout. This fixture intentionally exercises
    # the latter, but must still provide the production model-config surface.
    runner.model_config = SimpleNamespace(
        hf_config=SimpleNamespace(model_type="generic_test_model")
    )
