...
def test_deepseek_runtime_is_fl_owned_and_vendor_scoped() -> None:
    checked = [
        ...
        *ROOT.joinpath("vllm_fl/kv_cache/ascend").rglob("*.py"),
        *ASCEND.joinpath("device").rglob("*.py"),
        ROOT / "vllm_fl/distributed/ascend_parallel_state.py",
        ROOT / "vllm_fl/distributed/ascend_utils.py",
        *ASCEND.joinpath("ops").rglob("*.py"),
        ...
        ROOT / "vllm_fl/dispatch/backends/vendor/ascend/patches/patch_deepseek_v4.py",
    ]
    for path in checked:
...
def test_deepseek_registry_and_dsa_registration_are_explicit() -> None:
    registry = _source("vllm_fl/dispatch/backends/vendor/ascend/patches/patch_deepseek_v4.py")
...
def test_active_ascend_linear_preserves_deepseek_wo_a_grouped_layout() -> None:
    patch = _source("vllm_fl/dispatch/backends/vendor/ascend/patch.py")
    assert "from vllm_fl.dispatch.backends.vendor.ascend.impl.linear import (" in patch
