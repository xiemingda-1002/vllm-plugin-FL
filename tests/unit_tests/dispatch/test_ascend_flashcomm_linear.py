...
@pytest.mark.parametrize(
    ("prefix", "direction"),
    [
        ("model.vision_tower.blocks.0.attn.qkv_proj", "column"),
        ("model.vision_model.blocks.0.attn.o_proj", "row"),
        ("model.multi_modal_projector.gate_up_proj", "column"),
        ("model.patch_merge_mlp.down_proj", "row"),
    ],
)
def test_flashcomm_selector_skips_rc1_multimodal_encoder_components(
    monkeypatch: pytest.MonkeyPatch, prefix: str, direction: str
) -> None:
    linear_op = importlib.import_module(f"{MODULE_PREFIX}.linear_op")
    group = SimpleNamespace(rank_in_group=1, world_size=2)
    monkeypatch.setattr(linear_op, "flashcomm1_configured", lambda: True)
    monkeypatch.setattr(linear_op, "shared_expert_dp_enabled", lambda: False)
    monkeypatch.setattr(linear_op, "get_tp_group", lambda: group)

    custom_op, rank, size = linear_op.get_parallel_op(
        False, prefix, object(), direction
    )
...
def test_flashcomm_multimodal_exclusion_is_component_not_substring_match(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    linear_op = importlib.import_module(f"{MODULE_PREFIX}.linear_op")
    group = SimpleNamespace(rank_in_group=1, world_size=2)
    ...
    custom_op, rank, size = linear_op.get_parallel_op(
        False,
        "model.layers.0.vision_model_projection.qkv_proj",
        object(),
        "column",
    )

    assert isinstance(custom_op, linear_op.SequenceColumnParallelOp)
    assert (rank, size) == (1, 2)


def test_flashcomm_decoder_selection_and_shared_expert_disable_are_unchanged(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    linear_op = importlib.import_module(f"{MODULE_PREFIX}.linear_op")
    group = SimpleNamespace(rank_in_group=1, world_size=2)
    ...
    decoder, rank, size = linear_op.get_parallel_op(
        False, "model.layers.0.self_attn.qkv_proj", object(), "column"
    )
    shared, shared_rank, shared_size = linear_op.get_parallel_op(
        False,
    ...
    assert isinstance(decoder, linear_op.SequenceColumnParallelOp)
    assert (rank, size) == (1, 2)
    assert shared is None
    assert (shared_rank, shared_size) == (0, 1)
