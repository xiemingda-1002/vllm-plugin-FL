...
def test_cache_closure_is_vendor_scoped_and_non_packed() -> None:
    source = (ROOT / "vllm_fl/dispatch/backends/vendor/ascend/core/deepseek_v4_kv_cache.py").read_text()
...
def kv_module():
    try:
        from vllm_fl.dispatch.backends.vendor.ascend.core import deepseek_v4_kv_cache
    except ModuleNotFoundError as error:
        pytest.skip(f"requires vLLM test dependencies: {error}")
