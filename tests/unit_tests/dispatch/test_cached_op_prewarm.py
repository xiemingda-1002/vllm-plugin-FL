...
def isolated_cached_ops(monkeypatch):
    ...
    # This suite exercises the Ascend fail-closed protocol, independently of
    # the host vendor. Non-Ascend behavior has separate isolation coverage.
    monkeypatch.setattr(dispatch, "_requires_compile_prewarm", lambda: True)
...
def test_prewarm_resolves_and_records_first_use_without_execution(
    ...
    executions = []
    manager = FakeManager(
        {"op": _impl("vendor.op", lambda x: executions.append(x) or x)}
    )
