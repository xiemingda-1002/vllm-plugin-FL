...
def test_patch_op_cls_registers_layernorm_only_in_ascend_vendor(
    ...
    class AscendMultiHeadLatentAttention:
        pass
    ...
    monkeypatch.setitem(
        ...
        _stub_module(
            "vllm_fl.dispatch.backends.vendor.ascend.ops.mla",
            AscendMultiHeadLatentAttention=AscendMultiHeadLatentAttention,
    ...
    pluggable_registrations = {
        call.kwargs["name"]: call.kwargs["_decorated_layer_cls"]
        for call in register_pluggable.call_args_list
    }
    assert register_pluggable.call_count == 3
    assert pluggable_registrations == {
        "VocabParallelEmbedding": sys.modules[
            f"{module_prefix}.vocab_parallel_embedding"
