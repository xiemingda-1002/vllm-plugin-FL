...
def _install_config(monkeypatch, additional_config: dict) -> None:
    ...
    monkeypatch.setattr(compat, "shared_expert_dp_enabled_for_config", lambda _: False)


def _import_sfa(monkeypatch):
    """Import the backend with the same initialized device-type precondition."""
    from vllm_fl.dispatch.backends.vendor.ascend import dsa_compat, hardware
...
@pytest.mark.parametrize(
    "backend_path",
    [
        "vllm_fl.dispatch.backends.vendor.ascend.attention.sfa_v1.AscendSFABackend",
        "vllm_fl.dispatch.backends.vendor.ascend.attention.indexer.AscendSFAIndexerBackend",
    ],
)
