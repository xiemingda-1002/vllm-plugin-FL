...
def test_runtime_backend_selection():
    ...
    assert isinstance(get_graph_runtime_backend("npu"), AscendGraphRuntimeBackend)
...
def test_ascend_updates_attention_params_after_graph_forward(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda stream, context, num_tokens, config: update_calls.append(
            (stream, context, num_tokens, config)
        ),
    )
    ...
    backend.after_model_forward(vllm_config)

    assert update_calls == [(backend._update_stream, forward_context, 4, vllm_config)]
