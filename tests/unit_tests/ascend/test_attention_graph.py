...
def test_pa_capture_uses_max_workspace_sequence_length(monkeypatch):
...
def test_pa_capture_does_not_change_non_npu_sequence_length(monkeypatch):
    import vllm_fl.worker.model_runner as model_runner
    ...
    assert model_runner._ascend_graph_capture_seq_len(
        num_tokens=32,
        vllm_config=_config(pa_shape_list=[32]),
        is_graph_capturing=True,
        default_seq_len=1,
