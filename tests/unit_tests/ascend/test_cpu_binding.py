...
import vllm_fl.cpu_binding as cpu_binding_module
...
class TestDeviceInfo(unittest.TestCase):
    @patch("vllm_fl.cpu_binding.subprocess.Popen")
    ...
    @patch("vllm_fl.cpu_binding.execute_command")
    ...
    @patch("vllm_fl.cpu_binding.ASCEND_RT_VISIBLE_DEVICES", "1,5")
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_get_running_npus_filters_invalid_rows_and_visible_devices(
        self, mock_execute_command
    ):
    ...
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_get_running_npus_skips_non_pipe_rows_inside_process_section(
        self, mock_execute_command
    ):
        device_info = object.__new__(DeviceInfo)
    ...
    def test_parse_topo_affinity(self, mock_execute_command):
        mock_execute_command.return_value = (
            "NPU0 X HCCS HCCS HCCS HCCS HCCS HCCS HCCS 0-3",
            0,
        )
    ...
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_parse_topo_affinity_skips_affinity_header_and_non_npu_rows(
        self, mock_execute_command
    ):
        device_info = object.__new__(DeviceInfo)
        mock_execute_command.return_value = (
    ...
    @patch("vllm_fl.cpu_binding.os.path.exists", return_value=False)
    def test_parse_allowed_cpus_returns_empty_when_status_file_missing(
        self, _mock_exists
    ):
        device_info = object.__new__(DeviceInfo)
    ...
    @patch("vllm_fl.cpu_binding.os.path.exists", return_value=True)
    @patch(
        "builtins.open", new_callable=mock_open, read_data="Name:\tpython\nState:\tR\n"
    )
    def test_parse_allowed_cpus_raises_when_field_missing(
        self, _mock_open, _mock_exists
    ):
        device_info = object.__new__(DeviceInfo)
...
class TestCpuAlloc(unittest.TestCase):
    ...
    @patch("vllm_fl.cpu_binding.get_ascend_device_type")
    ...
    def test_build_cpu_pools_fallback_to_global_slice(self, mock_get_device_type):
        ...
        with (
            ...
            ) as mock_build_cpu_node_map,
            patch.object(
                self.cpu_alloc, "build_global_slice_cpu_pool"
            ) as mock_build_global_slice_cpu_pool,
        ):
    ...
    def test_build_global_slice_cpu_pool_splits_same_cpuset_across_processes(self):
        ...
        self.assertFalse(
            set(npu0_process.npu_cpu_pool[0]) & set(npu3_process.npu_cpu_pool[3])
        )
        self.assertFalse(
            set(npu0_process.assign_main[0]) & set(npu3_process.assign_main[3])
        )
    ...
    @patch("vllm_fl.cpu_binding.get_ascend_device_type")
    @patch("vllm_fl.cpu_binding.os.listdir")
    @patch(
        "builtins.open",
        new_callable=mock_open,
        read_data="123: 0 0 0 0 sq_send_trigger_irq\n",
    )
    @patch("vllm_fl.cpu_binding.shutil.which")
    @patch("vllm_fl.cpu_binding.os.access")
    @patch("vllm_fl.cpu_binding.execute_command")
...
class TestCpuBindingSupplemental(unittest.TestCase):
    ...
    @patch("vllm_fl.cpu_binding.execute_command", return_value=("ok", 0))
    ...
    @patch("vllm_fl.cpu_binding.execute_command", return_value=("failed", 1))
    ...
    @patch("vllm_fl.cpu_binding.get_ascend_device_type", return_value="unknown")
    def test_binding_mode_defaults_to_topo_affinity_for_unknown_device(
        self, _mock_get_device_type
    ):
        self.assertEqual(CpuAlloc._binding_mode(), "topo_affinity")

    @patch(
        "vllm_fl.cpu_binding.get_ascend_device_type", return_value=AscendDeviceType.A2
    )
    ...
    def test_build_cpu_pools_topo_mode_builds_and_splits_duplicate_groups(
        self, _mock_get_device_type
    ):
        cpu_alloc = make_cpu_alloc()
    ...
    def test_build_cpu_pools_topo_mode_skips_non_running_npu_without_cpuset_overlap(
        self, _mock_get_device_type
    ):
        cpu_alloc = make_cpu_alloc()
    ...
    def test_build_cpu_pools_topo_mode_excludes_non_running_npu_from_final_pool(
        self, _mock_get_device_type
    ):
        cpu_alloc = make_cpu_alloc()
        cpu_alloc.device_info.all_logic_npus = [0, 1]
    ...
    def test_build_cpu_pools_topo_mode_splits_hidden_same_affinity_npus_across_processes(
        self, _mock_get_device_type
    ):
        def build_single_card_process(visible_npu):
            cpu_alloc = make_cpu_alloc()
            ...
            with (
                patch.object(cpu_alloc, "build_cpu_node_map"),
                patch.object(
                    cpu_alloc, "extend_numa", return_value=list(range(144, 192))
                ),
            ):
                cpu_alloc.build_cpu_pools()
                cpu_alloc.allocate()
        ...
        self.assertFalse(
            set(npu0_process.npu_cpu_pool[0]) & set(npu2_process.npu_cpu_pool[2])
        )
        self.assertFalse(
            set(npu0_process.assign_main[0]) & set(npu2_process.assign_main[2])
        )
        self.assertFalse(
            set(npu0_process.assign_acl[0]) & set(npu2_process.assign_acl[2])
        )
        self.assertFalse(
            set(npu0_process.assign_rel[0]) & set(npu2_process.assign_rel[2])
        )

    @patch("vllm_fl.cpu_binding.logger.info")
    ...
    @patch("vllm_fl.cpu_binding.shutil.which", return_value=None)
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_bind_memory_skips_when_migratepages_missing(
        self, mock_execute_command, _mock_which
    ):
        cpu_alloc = make_cpu_alloc()
    ...
    @patch("vllm_fl.cpu_binding.shutil.which", return_value="/usr/bin/migratepages")
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_bind_memory_skips_when_cpu_pool_or_numa_invalid(
        self, mock_execute_command, _mock_which
    ):
        cpu_alloc = make_cpu_alloc()
    ...
    @patch("vllm_fl.cpu_binding.shutil.which", return_value="/usr/bin/migratepages")
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_bind_memory_executes_on_valid_numa_target(
        self, mock_execute_command, _mock_which
    ):
        cpu_alloc = make_cpu_alloc()
        ...
        mock_execute_command.assert_called_once_with(
            ["migratepages", "1000", "0,1", "1"]
        )

    @patch("vllm_fl.cpu_binding.psutil.Process")
    @patch(
        "vllm_fl.cpu_binding.execute_command",
        return_value=(
            "1000 2000 ? 00:00:01 acl_thread\n1000 3000 ? 00:00:01 release_thread",
            0,
        ),
    )
    def test_bind_threads_binds_main_acl_and_release_threads(
        self, _mock_execute_command, mock_process
    ):
        cpu_alloc = make_cpu_alloc()
        cpu_alloc.device_info.running_npu_list = [0]
        ...
        with (
            ...
            patch.object(cpu_alloc, "bind_memory") as mock_bind_memory,
        ):
    ...
    @patch("vllm_fl.cpu_binding.os.access", return_value=False)
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_bind_npu_irq_returns_when_irq_path_not_writable(
        self, mock_execute_command, _mock_access
    ):
        cpu_alloc = make_cpu_alloc()
    ...
    @patch("vllm_fl.cpu_binding.os.access", return_value=True)
    @patch("vllm_fl.cpu_binding.execute_command")
    ...
    @patch(
        "builtins.open",
        new_callable=mock_open,
        read_data="123: 0 0 0 sq_send_trigger_irq\n",
    )
    @patch("vllm_fl.cpu_binding.shutil.which", return_value=None)
    @patch("vllm_fl.cpu_binding.os.access", return_value=True)
    @patch("vllm_fl.cpu_binding.execute_command")
    ...
    @patch(
        "vllm_fl.cpu_binding.execute_command",
        return_value=("board info without pci", 0),
    )
    ...
    @patch("vllm_fl.cpu_binding.os.listdir", return_value=["456", "457"])
    @patch(
        "builtins.open",
        new_callable=mock_open,
        read_data="123: 0 0 0 sq_send_trigger_irq\n",
    ...
    @patch("vllm_fl.cpu_binding.os.listdir", return_value=["123", "124"])
    @patch(
        "builtins.open",
        new_callable=mock_open,
        read_data="123: 0 0 0 sq_send_trigger_irq\n",
    )
    @patch("vllm_fl.cpu_binding.shutil.which", return_value="/bin/systemctl")
    @patch("vllm_fl.cpu_binding.os.access", return_value=True)
    @patch("vllm_fl.cpu_binding.execute_command")
    def test_bind_npu_irq_stops_irqbalance_and_writes_affinity_masks(
        ...
        self.assertIn(
            call(["systemctl", "stop", "irqbalance"]),
            mock_execute_command.call_args_list,
        )
        self.assertIn(
            call(["npu-smi", "info", "-t", "board", "-i", "0"]),
            mock_execute_command.call_args_list,
        )
        handle = mock_file()
        self.assertEqual(
            handle.write.call_args_list, [call("00000100"), call("00000200")]
        )
    ...
    def test_bind_npu_irq_skips_irqbalance_handling_when_service_absent(
        ...
        self.assertNotIn(
            call(["systemctl", "is-active", "--quiet", "irqbalance"]),
            mock_execute_command.call_args_list,
        )
    ...
    def test_run_all_invokes_steps_in_order(self):
        ...
        with (
            patch.object(
                cpu_alloc,
                "build_cpu_pools",
                side_effect=lambda: calls.append("build_cpu_pools"),
            ),
            patch.object(
                cpu_alloc, "allocate", side_effect=lambda: calls.append("allocate")
            ),
            patch.object(
                cpu_alloc, "print_plan", side_effect=lambda: calls.append("print_plan")
            ),
            patch.object(
                cpu_alloc,
                "bind_threads",
                side_effect=lambda: calls.append("bind_threads"),
            ),
            patch.object(
                cpu_alloc,
                "bind_npu_irq",
                side_effect=lambda: calls.append("bind_npu_irq"),
            ),
        ):
            cpu_alloc.run_all()
...
class TestBindingSwitch(unittest.TestCase):
    @patch("vllm_fl.cpu_binding.platform.machine")
    ...
    @patch("vllm_fl.cpu_binding.CpuAlloc")
    @patch("vllm_fl.cpu_binding.is_arm_cpu")
    ...
    @patch("vllm_fl.cpu_binding.CpuAlloc")
    @patch("vllm_fl.cpu_binding.is_arm_cpu", return_value=True)
